...
class UserController extends Controller
{
    ...
    {
        $check = ProductWishlist::checkAlready($req->product_id, Auth::user()->id);
        if(empty($check))
        {
            $save = new ProductWishlist();
            $save->product_id = $req->product_id;
        ...
        {
            ProductWishlist::deleteRecord($req->product_id, Auth::user()->id);
    ...
    public function submit_review(Request $req)
    {
        ...
        $save->product_id = trim($req->product_id);
        $save->order_id = trim($req->order_id);
        $save->user_id = Auth::user()->id;
        $save->rating = trim($req->rating);
        $save->review = trim($req->review);
        $save->save();
